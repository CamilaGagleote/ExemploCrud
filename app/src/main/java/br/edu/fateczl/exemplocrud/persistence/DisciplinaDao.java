...
public class DisciplinaDao implements IDisciplinaDao, ICRUDDao<Disciplina> {
    ...
    public void insert(Disciplina disciplina) throws SQLException {
        ContentValues contenValue = getCOntentValues(disciplina);
        database.insert("disciplina", null, contenValue);
    }

    @Override
    public int update(Disciplina disciplina) throws SQLException {
        ContentValues contentValue = getCOntentValues(disciplina);
        int ret = database.update("disciplina", contentValue, "codigo = " +
    ...
    public Disciplina findOne(Disciplina disciplina) throws SQLException {
        String sql = "SELECT p.codigo AS cod.prof, p.nome AS nome_prof, p.titulacao AS tit_prof, " +
                "d.codigo, d.nome " +
                "FROM professor p, disciplina d " +
                "WHERE p.codigo = d.codigo_professo " +
    ...
    private ContentValues getCOntentValues(Disciplina d){
        ContentValues contentValue = new ContentValues();
        contentValue.put("codigo", d.getCodigo());
        contentValue.put("nome", d.getNome());
        contentValue.put("codigo_professor", d.getProfessor().getCodigo());
        return contentValue;
    }
}
